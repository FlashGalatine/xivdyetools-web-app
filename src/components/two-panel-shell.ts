/**
 * XIV Dye Tools v3.0.0 - Two Panel Shell Component
 *
 * Production two-panel layout shell for the v3.0.0 UI.
 * Desktop (≥768px): Left sidebar (nav + config) | Right content (results)
 * Mobile (<768px): Full-width content + Drawer trigger + Bottom nav
 *
 * @module components/two-panel-shell
 */
...
export interface TwoPanelShellOptions {
  initialTool?: ToolId;
  onToolChange?: (toolId: ToolId) => void;
}

/**
 * Main shell component for the two-panel v3 layout
 */
export class TwoPanelShell extends BaseComponent {
  private options: TwoPanelShellOptions;
  ...
  constructor(container: HTMLElement, options: TwoPanelShellOptions = {}) {
    ...
    const storedCollapsed = StorageService.getItem<boolean>('v3_sidebar_collapsed');
  ...
  private renderMobileBottomNav(): void {
    ...
    tools.forEach(tool => {
      ...
      btn.addEventListener('click', () => this.handleToolSelect(tool.id));
    ...
    if (this.isCollapsed) {
      ...
      tools.forEach(tool => {
        ...
        const btn = this.createElement('button', {
          ...
          innerHTML: `<span class="w-5 h-5">${TOOL_ICONS[tool.id]}</span>`,
        });
  ...
  private updateMobileDrawerNav(): void {
    ...
    tools.forEach(tool => {
      ...
      btn.setAttribute('style', isActive
        ...
        : 'background: transparent; color: var(--theme-text);');
      btn.innerHTML = `
        <span class="w-5 h-5 flex-shrink-0">${TOOL_ICONS[tool.id]}</span>
  ...
  toggleCollapse(): void {
    ...
    // Persist state
    StorageService.setItem('v3_sidebar_collapsed', this.isCollapsed);
