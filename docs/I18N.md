# XIV Dye Tools – Internationalization (i18n) Guide
...
```typescript
// ✅ Good - descriptive, follows namespace
...
The project includes a custom ESLint rule that warns against fallback patterns:

```typescript
// ⚠️ This triggers an ESLint warning:
LanguageService.t('some.key') || 'Fallback text'
...
The rule `xivdyetools-i18n/no-i18n-fallback` runs automatically with `npm run lint`.

### Running the Validator

The project also includes a build-time validation script:
...
📚 Loaded 988 translation keys from en.json
📁 Scanning 137 source files...

✅ All 967 translation key references are valid!

📊 Summary:
   • Total keys in en.json: 988
   • Keys referenced in code: 676
   • Total references checked: 967
```
...
- `docs/STYLE_GUIDE.md` – Overall front-end patterns
- `xivdyetools-docs/audits/2026-01-17/` – i18n audit findings and remediation
